...
- **proxy_advise 2단계 perf 조기 발사 (2026-06-10)** — 복합툴 6.4초 중 1차(8개)+2차(perf 3개)가 순차. perf는 1차 결과 무관(company_query만)이고 director_eval의 inside_renewed gate만 필요 → director를 1차에서 분리해 먼저 await, gate 판단 후 perf를 1차 완료 전 조기 발사(나머지 1차와 겹침). 모델 `현재 L1+P → max(L1,D+P)`, D≤L1이라 회귀 수학적 불가능. KOSPI 100 전수조사: renewed 64%, 회귀 0/100, 이득 중앙 1687ms(최대 3291). before/after `agenda_decisions`/`candidates_evaluations` bit-identical 확인. ([[lessons/proxy-advise-stage2-parallel-260610]])
