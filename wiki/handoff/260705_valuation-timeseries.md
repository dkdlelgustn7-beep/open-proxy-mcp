...
**한 줄(260706 갱신)**: FY0+TTM/MRQ 시장·섹터 과거 밴드 **완성·검증·배포**. 분기 백필(mkt_fund_q)
57,178건 **100% 완료**. sector scope에 소속 섹터 히스토리 추가. 통화통일 실험은 여전히 롤백 상태(미해결,
근본해법 필요). 스케일오류 5건 발견·수정. 자세한 건 아래 섹션 참조.

## 🟡 연간 백필(FY2025 신규 + 결측 90건) — 260706 사용자 지시로 중단, 재개 대기

**`market_val_series.py --fetch`를 사용자 지시("백필 종료")로 안전 종료(pkill, 완전종료 확인함).**
목적: ① `mkt_fund_hist`에 FY2025 연간자료 신규 수집 ② 이전에 'nodata'로 잘못 표시됐던 90건 재수집.
90건은 이미 DELETE해서 재시도 대상으로 만들어둔 상태(코드 자체 수정은 아님) — 아직 안 끝났으니 이 갭은
DB에 남아있음.

- **중단 시점 진행률**: `mkt_fund_hist` **16,268/20,792 (78.2%) · 잔여 4,524건**.
- **재개 방법**(resume 안전 — done-set이 이미 받은 건 자동으로 건너뜀):
  ```bash
  cd /Users/marcoyou/Projects/open-proxy-mcp && SERIES_SLEEP=0.2 python3 scripts/market_val_series.py --fetch
  ```
  (`SERIES_SLEEP` 환경변수는 이번에 코드에 추가함, 기본값 0.45·순차 실행이라 0.2도 안전 확인됨 —
  이 실행 전체에서 ReadError 등 에러 0건.)
- **완료되면**: `python3 scripts/market_val_history_backfill.py`를 한 번 더 돌려서 FY2025 반영된
  최신 밴드로 갱신(멱등, ON CONFLICT라 안전). 이후 "20260529·20260626 스냅샷 NULL" 문제도 자동
  해소되는지 확인.
- 백그라운드 프로세스·Monitor 전부 정리 완료, 세션 깨끗하게 종료됨.
...
- **분기 백필 100% 완료**(57,178/57,178) — 4분기(사업보고서) 커버리지도 검증(단, 아래 "작음" 항목 참조).
- **market/sector TTM/MRQ 과거 밴드 추가**(`market_val_history_backfill.py` 확장) — FY0에 이어 TTM/MRQ도
  76~78개월 시계열로 저장. `_pit_quarter`/`_ttm_ni`/`_mrq_eq`를 valuation.py에서 재사용(로직 이중구현 방지).
- **버그 발견·수정 2건**(둘 다 "지표별 독립 시총분모 페어링" 원칙 위반):
  1. TTM/MRQ 추가 시 논리 실수로 FY0 없는 종목 시총까지 분자에 섞임(2020-12 KOSPI FY0 PER 42.8로 왜곡) →
     4개 지표(per_fy0/pbr_fy0/per_ttm/pbr_mrq) 완전 독립 분모로 재작성.
  2. 위 수정 중 **원래 FY0 계산에도 같은 종류의 작은 버그가 있었음**을 발견(ni 결측·eq만 있는 40종목의
     시총이 분자에 섞임, 32.72→31.09로 정정) — 처음부터 있던 잔여 버그, 이번에 완전 해소.
  3. `mkt_fund_q` 실제 스케일오류 5건(007720·032080·060310·069330·323350, ×100만 단위오류)이 KOSDAQ
     시장/섹터 집계를 오염 → 무효화(`fetched='err:scale'`) 후 재계산으로 해소(재수집 대상으로 표시됨).
- **sector scope에 소속 섹터 히스토리 추가**(`build_sector_val_payload`) — company 지정 시 그 기업 섹터의
  78개월 시계열(`data.company.sector_history`)을 추가 반환(0콜, 이미 DB에 있던 데이터 연결만). QA 검증
  통과(대형 섹터·`_fold` 소규모 섹터 폴백 케이스 둘 다 확인, payload 크기 문제 없음).
- **6인 페르소나 워크플로우 E2E 리뷰** 수행(펀드매니저·애널리스트·스튜어드십·빌더·QA·재무분석가, DART
  콜 1건만 사용) — 나머지 미해결 이슈는 아래 "다음 작업" 참조.

### 작음 — 지금 넘어가도 됨(사용자 확인 260706)
- **fy2019 연간(hist) 진짜 갭 90건**(전체 57,178건 중 <0.2%) — 분기엔 실제값 있는데 연간 hist만
  'nodata'로 잘못 기록된 케이스(예: 삼양홀딩스). 처음엔 "100% 완료"로 잘못 보고했었음(QA agent가
  1,848건이라 과대추정한 걸 90건으로 정밀 재확인) — 작지만 실재, 후속 백필 때 정리.
- **20260529·20260626 스냅샷 FY0/TTM 전체 NULL** — 원인 규명 완료: 통화롤백과 무관, **mkt_fund_hist에
  FY2025 연간자료가 아직 없어서**(4월 이후 월말은 FY2025를 요구하는데 아직 미수집). DART 필요해 지금
  못 채움 — 매달 하나씩 늘어나는 구멍이니 다음 배포 사이클에 "FY2025 연간 mkt_fund_hist 백필" 항목 등록.
- `_fold`(소규모) 섹터 자체의 과거 히스토리 백필 안 됨(오늘 스냅샷 1개월치뿐) — sector_history 폴백은
  정상 동작하나 데이터 자체가 짧음. 후속 백필 대상.
...
- **260706 후속 발견·수정**: TTM/MRQ 밴드 추가 작업 중 "지표별 독립 분자(cap) 페어링" 원칙을 전면
  적용하면서, **원래 FY0 계산에도 작지만 같은 종류의 버그가 있었음**을 발견 — ni(순이익)가 결측인데
  eq(자본)만 있는 40개 종목(중단영업 등 케이스)의 시총이 PER 분자에 잘못 포함돼 있었음(2020-12
  KOSPI 예시: 32.72 → 정정 후 31.09, 종목수 520→480). TTM/MRQ 확장 때 신규로 219개 종목(시총 426조)이
  같은 방식으로 더 크게 오염되는 걸 잡으면서 기존 잔여분도 함께 해소됨. **market_val_history_backfill.py
  재실행 완료, 4개 지표(per_fy0/pbr_fy0/per_ttm/pbr_mrq) 전부 독립 시총분모로 재계산됨.**
