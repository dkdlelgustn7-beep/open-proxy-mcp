...
## ✅ 연간 백필(FY2025 신규 + 결측 90건) — 260706 완료

`market_val_series.py --fetch` 최종 완료: **`mkt_fund_hist` 20,792/20,792 (100%)**. FY2025 전 종목
(2,599사) 확보, 이전 'nodata' 오표시 90건도 재수집 완료. 재개 이력: 78.2%에서 중단 → `SERIES_SLEEP=0.2`
로 재개 → 100% 완주(전체 실행 중 ReadError 등 실질 에러 0건, 일시적 재시도 1건은 스크립트 자체 복구).

- **후속 조치 완료**: `market_val_history_backfill.py` 재실행 → 시장 156행 + 섹터 11,014행(78개월)으로
  FY2025 반영 갱신.
- **20260529·20260626 스냅샷 NULL 문제 완전 해소 확인** — per_fy0/per_ttm/pbr_fy0/pbr_mrq 4개 지표
  전부 정상 값으로 채워짐(예: 20260626 KOSPI PER(FY0) 107.97/TTM 60.12/PBR(FY0) 1.97/MRQ 1.93).
  원인이었던 "mkt_fund_hist에 FY2025 없음" 문제가 근본 해결돼 앞으로 매달 새로 생기던 구멍도 막힘.
- **남은 사소한 항목**: `_fold`(소규모) 섹터 자체의 과거 히스토리 백필은 여전히 미비(오늘 스냅샷뿐) —
  낮은 우선순위, 다음에 필요시 처리.
