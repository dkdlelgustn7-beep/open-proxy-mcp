...
### `mkt_valuation`(종목별 주간 스냅샷) — 2순위 증가 후보, 용도 미확인

260705 신설(현재 1주치 2,599행)이라 아직 작지만, **종목 단위**라 시장/섹터 집계보다 훨씬 빨리 커짐 —
주 2,599행 추정 시 **연 ~43MB**(krx_weekly보다 빠름). firm_history는 이미 mkt_fund_hist/q로 계산되는데
이 테이블의 과거 시계열이 실제로 쓰이는 곳이 있는지 **다음 세션에서 확인 필요** — 안 쓰이면 "최신 스냅샷만
유지"로 설계 고정 검토.
