...
- **결정↔실행결과 사이클 매칭(`_link_cycles`) 오탐 4건(260707 발견, 단위버그와 별개)** —
  POSCO홀딩스·카카오·엘앤에프·포스코퓨처엠에서 실행결과 금액이 매칭된 decision과 극단적으로
  안 맞음(원문 대조로 ACODE 단위배수 자체는 정상 확인, 날짜기반 매칭 로직이 엉뚱한 결정과 결과를
  짝지은 것으로 추정). 로컬 `TO_DO.md`에 기록, 별도 조사 필요.
