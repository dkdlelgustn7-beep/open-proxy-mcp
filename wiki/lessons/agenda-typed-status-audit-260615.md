...
- 🟡 **한국금융지주 — 미해결(별개 구조).** '목적사항별 기재사항' 섹션 **자체가 없음**(0회,
  원문 24KB). 정관 비교표(`구분/변경전/변경후` + `[집중투표제 도입ㆍ배제 세부내역]`)가
  소집공고 본문에 **인라인**으로 들어가 `parse_agenda_details_xml`이 0 detail 반환. 고치려면
  섹션 없을 때 전체 문서에서 비교표를 찾는 인라인 fallback이 필요한데 false positive 위험이
  커 별도 작업으로 분리.
