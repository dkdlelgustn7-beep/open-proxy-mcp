...
#### 🔴 진단 시 production 경로를 우회하지 말 것 (이번 오진의 근본 원인)
추적 중 "한국금융지주 = 섹션 부재"라고 한 번 **오진**했다. 원인을 정확히 분류하면:
- **tool 정의 문제 아님** — production tool(`build_shareholder_meeting_payload`)은 공시 선택
  로직이 정확해 올바른 소집공고(…000993)를 집고 있었다.
- **진단 스크립트의 자의적 판단** — tool을 우회하고 일회용 probe를 직접 짜면서
  `keywords=['소집']`(넓음) + `[0]`(검증 없이 첫 결과 채택)으로 **소집결의(…801215)**를 집었다.
  즉 tool엔 없는 버그를 진단 스크립트에서 만들어내고, 그 잘못된 입력으로 결론을 냈다.
원칙:
- **이미 production 경로(build_…_payload 등)가 있으면 진단도 그 경로로 한다.** raw API/rcept를
  직접 뒤지면 tool이 이미 해결한 선택·정규화 로직을 우회해 *진단 전용 버그*를 만든다.
- **probe 입력을 결론 근거로 쓰기 전에 sanity check.** 공고 html이 24KB(정상 100~400KB)면
  "구조가 다르네"가 아니라 "잘못된 문서를 집었나?"를 먼저 의심. 크기·건수 이상은 신호다.
- 회귀 검증을 코드 수정엔 적용하면서 **조사 단계의 입력 검증은 건너뛰는** 비대칭을 경계.

### KOSPI 전수조사 결론 (485사, 리츠 제외, 2026-06-15)

`scripts/aoi_kospi_census.py` — production 경로(build_…_payload scope=full)로 KOSPI500−리츠
전수. raw: [[260615_aoi_kospi_census]].
- 정관 안건 보유 **446사 중 파싱 실패 단 1사(0.2%) = 한국금융지주**.
- **기업은행은 통과**(KOSPI500 rank 54, 실패목록 없음) → fix(`e781989`)가 전수에서 검증됨.
- 유일 잔존 = 한국금융지주 **단일library_섹션드롭** 패턴. "패턴들"이라 할 게 없을 만큼
  수렴 — 1사 위해 공용 파서(`_parse_library_block`) 수술은 비효율, 한다면 그 1사류 한정
  LLM 폴백이 안전(공용 파서 불변). **보강 결정: 현 상태 유지, 한국금융지주는 알려진 한계로 둠.**

#### 도메인 메모
- **probe 시 소집공고 vs 소집결의 구분**: 같은 날 둘 다 제출되며 키워드 '소집'이 둘 다 매칭.
  결의(…8xxxxx)는 짧은 결정공시, 공고(…0xxxxx)가 목적사항별 기재사항 본문.
...
## 보수한도 단일-library fallback (2026-06-17) — 기업은행·한국금융지주 amount_unparsed 해결

KOSPI 보수한도 census에서 기업은행·한국금융지주가 `amount_unparsed`(안건은 잡혔는데 당기/전기
금액 빈손)였다. 원인 = **위 정관 파싱 실패와 동일 뿌리: 전 안건을 단일 `<library>`에 몰아넣는
양식** → 보수 안건 detail에 당기/전기 표가 정상 귀속 안 됨.

- **금융사 전반 아님 확인** (금융 35사 전수, [[260617_fin_library_census]]): 단일 library는
  이 2사뿐, 나머지 33사는 library 4~7개로 정상 ok. 업종 특성 아닌 **두 회사 공시 템플릿** 문제.
- **fix = 원문 텍스트 fallback** (`_compensation_raw_fallback`): 구조 파싱이 **양쪽 다 빈손일
  때만** 발동(gating) → 정상 33사 미접촉(회귀 안전 by construction). (당 기) 마커를 순회하며
  직전 라벨(이사/감사)로 분류 후 '보수총액 또는 최고한도액'(당기) / '최고한도액'(전기, 실제지급
  제외) 추출. 외화는 환산 None.
  - 검증: 기업은행 이사 12.16→12.51억·감사 3.34→3.44억(상향), 한국금융지주 이사 50→100억
    (상향). 금융 35사: 2사 ok 전환, 33사 status 불변. 전체 82테스트 통과.
  - → KOSPI 보수한도 통계 판정불가 2사 해소: **상향 23 / 유지 72 / 하향 5 / 판정불가 0**.
- 교훈: "스코프 첫 매치" 함정 — `이사 보수` 첫 매치가 상단 '사외이사 보수현황' 섹션을 잡아
  당기/전기 없는 곳을 스코프(초기 실패). **(당 기) 마커 자체를 앵커로** 쓰니 안건 표에만 적중.
