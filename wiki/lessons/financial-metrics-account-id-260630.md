...
3. **EPS도 같은 클래스(260701 후속 점검에서 발견)** = 주당이익은 주요계정에 없어 전체에서 오는데, nm
   ...
   **EPS가 FnGuide와 수%p 다를 수 있는 이유(버그 아님)**: 우리 tool은 **DART 공시 원문의 기본주당이익**
   (`ifrs-full_BasicEarningsLossPerShare`, 회사가 감사받아 신고한 값 — 가중평균 **유통**주식수=자기주식 제외 기반)을
   그대로 보고한다. FnGuide는 **자체 재계산**(지배순이익÷다른 주식수 기준)이라 분모가 달라진다. 증거: 주식수가
   안 변한 해(삼성전자 2023·2024)는 **정확히 일치**(2131/4950), **자사주 매입·소각이 있던 해**(삼성전자 2025
   6605 vs 6564, 현대모비스 전기간 ~3%)만 벌어짐 = 가중평균 유통주식수 처리 차이. → **우리 값(공시 신고
   EPS, 자기주식 제외)이 정석**이며 FnGuide와의 소차는 정의 차이지 오류가 아니다. (DPS·PER·BPS도 산정기준
   다르면 기관별 소차 가능 — 같은 원리.)
