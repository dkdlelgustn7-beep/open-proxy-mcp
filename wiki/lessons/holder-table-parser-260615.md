...
### 잔여 ~5.8%(16/277) 분류 (미해결 — verified=False로 정직 표기)
- **합 ≫ 헤드라인 (과다)**: 정정본 미분리·합계표 아닌 보유표 오매칭 — 미래에셋벤처투자 69 vs
  5.1, 레인보우 82 vs 58.6, 산돌 +9.9.
- **합 ≪ 헤드라인 (누락)**: 영문명 보고자(파라택시스 4.5 vs 57.7)·표 레이아웃 변형(한화·클래시스).
- **all-dash 합계표**: 씨씨에스·시스웍 — 합계표 셀이 전부 "-"(실보유는 다른 표). 합 0.
- 전부 `co_holders_verified=False`로 잡혀 확정 인용 차단(honesty 유지). 케이스별 이질적이라
  단일 패치로 안 되고 표 레이아웃별 보강 필요 — 별도.
