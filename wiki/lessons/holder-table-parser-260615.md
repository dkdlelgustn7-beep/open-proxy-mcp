...
### 영문명 truncation 추가 수정 (2026-06-17)
긴 영문 펀드명이 첫 글자 잘려 매칭됐다("Align Partners Capital Management Limited"=41자 →
"lign…"). 원인: 이름 정규식 길이 상한 `{1,40}`이 41자보다 짧아, "A"에서 시작하면 40 초과로
실패하고 한 칸 밀린 40자 부분문자열에 매칭. **상한 40→60으로 해결**. 332사 재검증: 영문명
복원, 불변식 92.4%(255/276)·fail 2 불변(회귀 0), 파싱 +1.

### 잔여 ~7%(20/275) 분류 (미해결 — verified=False로 정직 표기)
- **합 ≪ 헤드라인 (대부분)**: 보고자 본인/대형주주 행 미파싱(표 레이아웃·ID 형식 변형) →
  포스코퓨처엠 2.4 vs 60.6, SK이노 0.01 vs 52 등.
- **합 ≫ 헤드라인**: 정정본 합계표 미분리(중복) 또는 합계표 아닌 보유표 오매칭 →
  미래에셋벤처투자 69 vs 5.1, 레인보우 82 vs 58.6.
- 전부 `co_holders_verified=False`로 잡혀 확정 인용은 차단됨(honesty 유지). 완전 해결은 별도.
